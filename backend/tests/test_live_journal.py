...
def test_closed_trade_summary_snapshot():
    ...
    legs = [
        SimpleNamespace(symbol="C-BTC-1", side="sell", qty=10, type="call", strike=1, expiry="2026-09-26",
                        entry=100, exit_price=150, exit_at=opened + timedelta(minutes=30),
                        exit_reason="SL exit", exit_gross=-0.5, exit_fees=0.02, stop_pnl=-0.4,
                        stop_price=140, status="closed"),
        SimpleNamespace(symbol="P-BTC-1", side="sell", qty=10, type="put", strike=1, expiry="2026-09-26",
                        entry=100, exit_price=60, exit_at=opened + timedelta(minutes=30),
                        exit_reason="SL exit", exit_gross=0.4, exit_fees=0.02, stop_pnl=None,
                        stop_price=None, status="closed"),
    ]
    pos = SimpleNamespace(id=uuid.uuid4(), name="BTC 2026-09-26 (live)", underlying="BTC",
                          expiry="2026-09-26", status="closed", opened_at=opened,
                          closed_at=opened + timedelta(minutes=30), close_reason="SL: leg TP/SL",
                          auto_exit=True, stop_loss_amount=1.0, stop_loss_pct_of_margin=None, legs=legs)
