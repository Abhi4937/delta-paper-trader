...
- **New behavior:** in builder mode the rows look **identical to view mode — price only, no B/S on any cell**. **Tapping a strike's Mark opens a small add-popover for ONLY that cell** (`activeCell` state) containing the leg label, a **lot stepper (`− N +`, default 1)**, and **`Buy`/`Sell`** buttons. Buy/Sell call `selectLeg(contract, side, lots)` (lots chosen up front, not just 1) and close. A full-screen backdrop dismisses on outside tap. Same behavior on desktop (click-to-open). Matches Delta's `SS/…when we click on leg…` plus up-front lot sizing.
